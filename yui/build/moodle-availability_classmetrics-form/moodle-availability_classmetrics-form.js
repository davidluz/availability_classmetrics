YUI.add('moodle-availability_classmetrics-form', function(Y, NAME) {
    ...
    M.availability_classmetrics.form.getNode = function(json) {
        ...
        ruleWrap.append('<label><input type="radio" name="rule" value="percent"' + (rule==='percent'?' checked':'') + '> ' + (strings.rule_percent || '% de conclusão (turma)') + '</label> ');
        ruleWrap.append('<label><input type="radio" name="rule" value="minstudents"' + (rule==='minstudents'?' checked':'') + '> ' + (strings.rule_minstudents || 'Nº mínimo de alunos (turma)') + '</label>');
        ...
        aggWrap.append('<label><input type="radio" name="agg" value="all"' + (aggregation==='all'?' checked':'') + '> ' + (strings.aggregation_all || 'TODAS') + '</label> ');
        aggWrap.append('<label><input type="radio" name="agg" value="any"' + (aggregation==='any'?' checked':'') + '> ' + (strings.aggregation_any || 'QUALQUER') + '</label>');
        ...
        // Alternância
        root.one('input[name=rule][value=percent]').on('change', function() {
            percentWrap.setStyle('display', '');
            minWrap.setStyle('display', 'none');
        });
        root.one('input[name=rule][value=minstudents]').on('change', function() {
    ...
    M.availability_classmetrics.form.fillValue = function(value, node) {
        ...
        if (rule === 'percent') {
            ...
            value.aggregation = node.one('input[name=agg]:checked').get('value') === 'any' ? 'any' : 'all';
...
M.availability_classmetrics.form.fillErrors = function(errors, node) {
    ...
    if (rule === 'percent') {
        ...
        if (selectedActs === 0) { errors.push('error_noactivities'); }

        var p = parseInt(node.one('.percentblock input[type=number]').get('value'), 10);
        if (isNaN(p) || p < 0 || p > 100) { errors.push('error_percent'); }
    } else {
        var m = parseInt(node.one('.minblock input[type=number]').get('value'), 10);
        if (isNaN(m) || m < 0) { errors.push('error_minstudents'); }
    }
